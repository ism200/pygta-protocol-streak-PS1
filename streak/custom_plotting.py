...
def _custom_cyclers_for_svd_residual():
    return (
        ...
        cycler_func(color=["tab:cyan"]),
        # cycler_func(color=["cc.cyan"]), # this fails
    )
...
# if it is possible to add arguments for the range(len(handles)) loop
# then this function is no longer needed
def plot_fitted_traces_iscience2(
    result: ResultLike,
    wavelengths: Iterable[float],
    axes_shape: tuple[int, int] = (4, 4),
    center_λ: float | None = None,
